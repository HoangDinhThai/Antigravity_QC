...
1. **CẤM TỰ Ý PUSH COMMENT TRỰC TIẾP (KỂ CẢ KHI ĐANG SỬA NỘI DUNG):**
   ...
   - **Trong quá trình chỉnh sửa / cập nhật nội dung task hoặc comment:** Tất cả chỉnh sửa CHỈ ĐƯỢC LƯU VÀO FILE NHÁP LOCAL (`backlog/draft/test-evidence-{issueKey}.md`). KHÔNG ĐƯỢC tự động push bất kỳ comment nào lên Backlog cho đến khi User phát lệnh push cụ thể.
...
4. **QUY TRÌNH 4 BƯỚC THỰC THI BẮT BUỘC:**
   - **Bước 1 (Drafting & Editing):** Soạn hoặc chỉnh sửa nội dung báo cáo ra file nháp local tại thư mục `backlog/draft/test-evidence-{issueKey}.md` (hoặc `test-evidence-{issueKey}-tc{X}.md`).
